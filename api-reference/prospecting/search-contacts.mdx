...
<!-- VERIFY: live site and openapi.json disagree here — docs.lusha.com/tutorials/v3-migration-guide documents a v3 replacement (POST /v3/contacts/prospecting) as current, with the legacy path sunsetting Nov 18, 2026, but openapi.json only defines the legacy POST /prospecting/contact/search contract the Mintlify playground binds to. Kept the legacy endpoint here since that's what actually executes; revisit once openapi.json adds v3 paths. -->
    ...
    <ParamField body="filters.contacts.include" type="object">
      ...
      <Expandable title="include fields">
        ...
        <ParamField body="filters.contacts.include.existing_data_points" type="string[]">
          Only return contacts that have specific data available. Example values: `"phone"`, `"work_email"`, `"mobile_phone"`, `"direct_phone"`. See [Contact filters](/filters/contact-filters) for the full list of valid values.
        </ParamField>
