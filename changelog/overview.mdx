...
- `POST /v3/companies/search-and-enrich` - returned automatically; this endpoint takes no `reveal` field
...
**Search & Enrich: `reveal` field description corrected** - The Search & Enrich tag description stated that both contact and company endpoints control output via a `reveal` field. That is true only for `POST /v3/contacts/search-and-enrich`. `POST /v3/companies/search-and-enrich` has no `reveal` field and reveals all available premium data points automatically. Behavior is unchanged - only the documentation was wrong.
