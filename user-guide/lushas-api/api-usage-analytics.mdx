...
- **Filter by API key** - click the **All API keys** dropdown on the right side of the API section and select a specific key, or keep it on All API keys for a full account view.
- **Choose a time range** - use the **Time range** selector (Daily, Weekly, Monthly) to see when credits were used.
- **Review the Endpoint column** - check the **Endpoint** column to see which specific actions (such as `/v2/person` or `/v2/company`) are driving consumption.
