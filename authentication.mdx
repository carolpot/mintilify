...
description: "Get your Lusha API key from the API dashboard and pass it in the api_key header on every request. Learn what to expect when authentication fails."
---
...
<Steps>
  ...
  <Step title="Open the API dashboard">
    Generate and retrieve your API key from the [Lusha API Dashboard](https://dashboard.lusha.com/enrich/api). Copy it to a secure location.
  </Step>
</Steps>
...
- Verify you are using the correct key from the [API dashboard](https://dashboard.lusha.com/enrich/api).
- Ensure the key has not been revoked or regenerated since you last copied it.

<Warning>
  Keep your API key secret. Do not commit it to version control, embed it in client-side code, or share it publicly. If a key is exposed, regenerate it immediately from the [API dashboard](https://dashboard.lusha.com/enrich/api).
</Warning>
