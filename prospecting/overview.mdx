...
<!-- VERIFY: live site and openapi.json disagree here — docs.lusha.com/tutorials/v3-migration-guide positions v3 (POST /v3/contacts/prospecting, POST /v3/contacts/enrich, POST /v3/companies/prospecting, POST /v3/companies/enrich) as the current prospecting API, with the legacy /prospecting/* endpoints documented below sunsetting Nov 18, 2026. openapi.json in this repo only defines the legacy paths, so this page (and its child pages) intentionally still document them. Revisit once openapi.json is updated with v3 paths. -->
