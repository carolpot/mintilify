...
<Columns cols={2}>
  ...
  <Card title="Decision Makers" icon="user-tie" href="/decision-makers/overview">
...
- **Waterfall Reveal** — an opt-in enhancement to contact enrichment. When Lusha's own data has no match for a field, pass `waterfallReveal` alongside `reveal` to fall through to your enabled third-party providers instead of returning an empty field. Requires Data Waterfall enabled under **Account > Waterfall** in the dashboard.
