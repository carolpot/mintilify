Redocly to Mintlify Migration Plan
...
Note: CS/Support team should review all User Guide articles before publication and own this section long-term.
...
Build full URL redirect map from old Redocly paths | Engineering | Critical before DNS cutover
Implement redirects in docs.json | Engineering |
Validate OpenAPI spec completeness against live API | Engineering |
Set up custom domain (docs.lusha.com to Mintlify) | Engineering / DevOps | DNS change + Mintlify domain config
...
Set up branch protection and PR workflow on main branch | Engineering |
...
Note: Assumes 1-2 people working part-time. Can be compressed by moving Phase 3 to a post-launch iteration.
